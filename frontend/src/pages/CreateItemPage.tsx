...
export default function CreateItemPage() {
  ...
  const [form, setForm] = useState({
    ...
    startPrice: '', reservePrice: '', bidIncrement: '1',
    startTime: '', endTime: '',
  })
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      const now = new Date()
      const start = form.startTime ? new Date(form.startTime) : new Date(now.getTime() - 3600 * 1000) // 默认1小时前开始
    ...
    onError: (err: Error) => setError(err.message || '创建失败'),
  })

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault(); setError('')
    if (!form.title || !form.startPrice || !form.endTime) { setError('请填写必填项'); return }
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl p-6 shadow-sm space-y-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">拍品标题 *</label>
          <input type="text" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })}
            className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500"
            placeholder="请输入拍品标题" required />
        </div>
        <div>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">起拍价(元) *</label>
            <input type="number" step="0.01" value={form.startPrice}
              onChange={(e) => setForm({ ...form, startPrice: e.target.value })}
              className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500" placeholder="0.01" required />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">保留价(元)</label>
            <input type="number" step="0.01" value={form.reservePrice}
              onChange={(e) => setForm({ ...form, reservePrice: e.target.value })}
              className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500" placeholder="不设保留价" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">加价幅度(元)</label>
            <input type="number" step="0.01" value={form.bidIncrement}
              onChange={(e) => setForm({ ...form, bidIncrement: e.target.value })}
              className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500" placeholder="默认 1.00" />
          </div>
        </div>
        ...
        <div className="flex gap-4 pt-4">
          <button type="button" onClick={() => navigate('/my-items')}
            className="flex-1 px-6 py-2 border rounded-lg hover:bg-gray-50">取消</button>
