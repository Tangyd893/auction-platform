...
import { itemApi, bidApi } from '../lib/api'
...
export default function ItemDetailPage() {
  ...
  const { data: itemData, isLoading } = useQuery({
    queryKey: ['item', id],
    queryFn: () => itemApi.get(Number(id)),
    refetchInterval: 5000, // 每5秒刷新
  })

  const { data: bidsData } = useQuery({
    queryKey: ['bids', id],
    queryFn: () => bidApi.getByItem(Number(id)),
  ...
  const bidMutation = useMutation({
    mutationFn: (amount: number) => bidApi.place(Number(id), amount),
    onSuccess: () => {
  ...
  const minBid = item ? item.currentPrice + item.bidIncrement : 0
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2 text-gray-600">
            <Clock size={18} />
            <span>
              截止时间：{new Date(item.endTime * 1000).toLocaleString()}
            </span>
          </div>
          <div className="flex items-center gap-2 text-gray-600">
            <User size={18} />
          ...
          {bids.length === 0 ? (
            ...
            <div className="space-y-2">
              {bids.slice(0, 5).map((bid: any, index: number) => (
                ...
                >
                  <span className="text-gray-700">买家 {bid.buyerId}</span>
                  <span
